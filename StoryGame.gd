...
func _ready() -> void:
	...
	GameData.current_stage = 121  # 신단수 1 = 스테이지 121
	
	# 배경 이미지 (챕터1: 항상 스토리 평원)
	var bg_path = "res://배경_스토리평원.png"
			...
			if enemies.size() > 0 and is_instance_valid(enemies[0]):
				for _i in range(population):
					if is_instance_valid(enemies[0]) and enemies[0].has_method("take_damage"):
						...
						if u["name"] == "돌칼병":
							if enemies[0].has_method("apply_burn"):
								enemies[0].apply_burn(GameData.get_total_attack() * 0.2, 5, false, 0, false)
							enemies[0].modulate = Color(1.0, 0.4, 0.4)
							var bvt = get_tree().create_timer(2.5)
							bvt.timeout.connect(func():
								if is_instance_valid(enemies[0]):
									enemies[0].modulate = Color(1.0, 1.0, 1.0)
							)
						elif u["name"] == "돌창병":
							_unit_apply_vulnerable(enemies[0], 5.0, 0.5)
							enemies[0].modulate = Color(1.0, 0.4, 0.4)
			...
			while count < population:
				...
				if is_instance_valid(e) and e.has_method("take_damage"):
					...
					if u["name"] == "돌칼병":
						...
						var spear_timer = get_tree().create_timer(5.0)
						spear_timer.timeout.connect(func():
...
func _on_stage3_complete() -> void:
	...
	_force_defeat()
	_show_story_dialog([
		"돌을 던지고 주먹을 휘둘러 보지만, 어둠은 끝도 없이 몰려온다.",
		"체력은 바닥나고, 희망도 사라져 간다.",
		"이대로 어둠 속으로 사라지는 것인가..."
	], _show_lightning_event)
