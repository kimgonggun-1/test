...
func _show_story_dialog(lines: Array, on_complete: Callable) -> void:
	...
	# 인라인 이미지 (특정 대사 옆에 표시)
	var inline_img = TextureRect.new()
	inline_img.size = Vector2(60, 60)
	inline_img.stretch_mode = TextureRect.STRETCH_KEEP_ASPECT_CENTERED
	inline_img.expand_mode = TextureRect.EXPAND_IGNORE_SIZE
	bg.add_child(inline_img)
	inline_img.visible = false
...
func _show_skill_complete_popup(skill_name: String, skill_img: String, on_complete: Callable, sub_text: String = "") -> void:
	var screen = get_viewport().get_visible_rect().size
	...
	# 텍스트
	var title = Label.new()
	title.text = "새로운 장비 [" + skill_name + "]을 습득하였습니다"
	title.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	...
	if sub_text != "":
		...
		sub_lbl.add_theme_color_override("font_color", Color(0.6, 1.0, 0.6))
		sub_lbl.add_theme_color_override("font_outline_color", Color(0,0,0))
...
func _show_chat_dialog(lines: Array, on_complete: Callable) -> void:
	...
	# 와이프 단독 이미지 (오른쪽)
	var wife_char = TextureRect.new()
	wife_char.position = Vector2(screen.x - 130, 10)
	wife_char.size = Vector2(110, 110)
	wife_char.stretch_mode = TextureRect.STRETCH_KEEP_ASPECT_CENTERED
	wife_char.expand_mode = TextureRect.EXPAND_IGNORE_SIZE
	if ResourceLoader.exists("res://스토리3_3.png"):
		wife_char.texture = load("res://스토리3_3.png")
	bg.add_child(wife_char)
	wife_char.visible = false
	...
	var update_line = func():
		...
		if speaker == "me":
			...
			wife_char.visible = true
			label.position = Vector2(30, 30)
			label.size = Vector2(screen.x - 160, 120)
		...
		# 인라인 이미지 표시 (대사 텍스트 바로 옆)
		if img_path != "" and ResourceLoader.exists(img_path):
			inline_img.texture = load(img_path)
			inline_img.visible = true
			if speaker == "wife":
				# 오른쪽 정렬: 이미지를 텍스트 영역 오른쪽 끝에 배치, 텍스트는 왼쪽으로 줄임
				label.size.x -= 70
				inline_img.position = Vector2(label.position.x + label.size.x + 10, 50)
			else:
				# 왼쪽 정렬: 이미지를 텍스트 영역 왼쪽 끝에 배치
				inline_img.position = Vector2(label.position.x, 50)
				label.position.x += 70
				label.size.x -= 70
...
func _start_story_chapter4() -> void:
	GameData.story_card_filter = "fist_only"
	_start_stage_sequence(1, 3, _on_stage3_complete)

func _on_stage3_complete() -> void:
	_force_defeat()
	_show_chat_dialog([
		{"speaker": "wife", "text": "어머, 당신, 손이 왜 이 모양이에요?"},
		{"speaker": "me", "text": "경작지에서 작물을 보고 있는데 들개들이 갑자기 공격해와서 맨손으로 싸우다 보니 이렇게 됐소.."},
		{"speaker": "me", "text": "경작지에 갈 때마다 무기를 들고 다닐 수도 없고, 그렇다고 매번 맨손으로 싸우려니 여간 힘든게 아니구려.."},
		{"speaker": "wife", "text": "뭐라도 손을 보호할 수 있는 물건이 있으면 좋겠어요."},
		{"speaker": "wife", "text": "지난번에 족장님과 함께 사냥한 큰 짐승의 가죽이 있지 않아요?"},
		{"speaker": "wife", "text": "그걸로 제가 한번 뭐라도 만들어 볼게요."},
		{"speaker": "wife", "text": "짠! 한번 손에 착용해 보세요.", "img": "res://가죽주먹.png"},
		{"speaker": "me", "text": "손에 딱 맞구려. 이런 도구가 있다면 앞으로는 들개놈들따위야, 맨손으로도 다 쓰러뜨릴 수 있겠소."},
		{"speaker": "wife", "text": "어머, 저게 뭐야? 들개들이 또 몰려오고 있어요!"},
		{"speaker": "wife", "text": "저것 좀 봐, 작물을 다 파헤치고 있잖아.."},
		{"speaker": "me", "text": "기다리시오. 내가 가서 처리하고 오리다."}
	], _start_fist_battle)

func _start_fist_battle() -> void:
	# 잔여 오버레이 정리
	for child in get_children():
		if child is CanvasLayer and child.layer >= 40:
			child.queue_free()
	# 4단계부터 가죽주먹 효과 임시 적용 (아이콘 변경 + 강타)
	GameData.has_leather_fist = true
	_start_stage_sequence(4, 10, _on_chapter4_complete)

func _on_chapter4_complete() -> void:
	_show_story_dialog([
		"이 가죽장갑이라면, 들개 따위는 두렵지 않다.",
		"이제 우리 가족과 작물을 지킬 수 있어."
	], func():
		_show_skill_complete_popup("가죽주먹", "res://가죽주먹.png", func():
			GameData.story_cleared["4"] = true
			GameData.story_card_filter = ""
			GameData.has_leather_fist = true
			GameData.save_game()
			get_tree().change_scene_to_file("res://scenes/lobby.tscn")
